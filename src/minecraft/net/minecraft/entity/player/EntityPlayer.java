...
public abstract class EntityPlayer extends EntityLivingBase implements ICommandSender
{
     ...
     */
    protected void damageEntity(DamageSource damage, float power)
    {
     	
    	if (damage.getDamageType().toString().equalsIgnoreCase("fall")){
    		
    		if (this.isPotionActive(Potion.nofall)){
    			return;
    		}
        ...
        if (!this.isEntityInvulnerable())
        {
            if (!damage.isUnblockable() && this.isBlocking() && power > 0.0F)
            {
                power = (1.0F + power) * 0.5F;
            }

            power = this.applyArmorCalculations(damage, power);
            power = this.applyPotionDamageCalculations(damage, power);
            float var3 = power;
            power = Math.max(power - this.getAbsorptionAmount(), 0.0F);
            this.setAbsorptionAmount(this.getAbsorptionAmount() - (var3 - power));

            if (power != 0.0F)
            {
                this.addExhaustion(damage.getHungerDamage());
                float var4 = this.getHealth();
                this.setHealth(this.getHealth() - power);
                this.func_110142_aN().func_94547_a(damage, var4, power);
            }
        }
    }
